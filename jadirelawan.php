...
if (!isset($_SESSION['id_user'])) {
    die("<div class='alert alert-danger text-center'>Silakan login terlebih dahulu.</div>");
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $id_user = $_SESSION['id_user']; // 🔴 WAJIB
    $nama   = htmlspecialchars($_POST['nama']);
    $email  = htmlspecialchars($_POST['email']);
    $alasan = htmlspecialchars($_POST['alasan']);

    // 🔴 id_user HARUS ikut di-insert
    $query = "INSERT INTO aktivitas (id_user, nama, email, alasan) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($koneksi, $query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "isss", $id_user, $nama, $email, $alasan);
        ...
        if ($result) {
            ...
            echo "<div class='alert alert-danger text-center'>Gagal mengirim formulir.</div>";
        }

        mysqli_stmt_close($stmt);
    } else {
        echo "<div class='alert alert-danger text-center'>Kesalahan server.</div>";
    }
}
?>
